import { claimDemoForUser } from "@/lib/claimDemo";
...
export async function getOrCreateBusiness(supabase, user) {
  ...
  // Signed up through a /claim/[token] link? Attach that prebuilt demo
  // instead of creating an empty business. Must happen before the upsert
  // below: owner_id is UNIQUE, so once this user owns an empty row the
  // demo can never be attached to them.
  const claimed = await claimDemoForUser(user);
