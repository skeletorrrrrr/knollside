...
export async function getPublicConfig(slug) {
  ...
  const { data: business, error: bErr } = await supabase
    .from("businesses")
    .select("id, name, slug, logo_url, industry, quantity_type, labor_rate, min_price, spread_pct, subscription_status, subscription_tier")
    .eq("slug", slug)
    .maybeSingle();
  ...
  return {
    business: {
      ...
      hide_branding: ["growth", "pro"].includes(business.subscription_tier),
    },
    items: items || [],
    options: options || [],
    addons: addons || [],
